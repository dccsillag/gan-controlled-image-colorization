...
class BaseModel:
    ...
    def build(self):
        ...
        # Make sure it is RGB and not Lab
        rgb_image = postprocess(tf.convert_to_tensor(gen), colorspace_in=self.options.color_space, colorspace_out=COLORSPACE_RGB)
        hsv_image = tf.image.rgb_to_hsv(rgb_image)
        self.dis_loss_saturation = tf.reduce_mean(tf.abs(hsv_image[:, :, 1] - self.options.desired_saturation))*self.options.saturation_weight
        self.dis_loss_hue = tf.reduce_mean(tf.abs(hsv_image[:, :, 0] - self.options.desired_hue))*self.options.hue_weight
        self.dis_loss_red = tf.reduce_mean(tf.abs(rgb_image[:, :, 0] - self.options.desired_red))*self.options.red_weight
        self.dis_loss_green = tf.reduce_mean(tf.abs(rgb_image[:, :, 1] - self.options.desired_green))*self.options.green_weight
        self.dis_loss_blue = tf.reduce_mean(tf.abs(rgb_image[:, :, 2] - self.options.desired_blue))*self.options.blue_weight
        ...
        self.accuracy = pixelwise_accuracy(self.input_color, gen, self.options.color_space, self.options.acc_thresh, options=self.options)
    ...
    def eval_outputs(self, feed_dic):
        ...
        lossD_saturation = self.dis_loss_saturation.eval(feed_dict=feed_dic)
        lossD_hue = self.dis_loss_hue.eval(feed_dict=feed_dic)
        lossD_red = self.dis_loss_red.eval(feed_dict=feed_dic)
        lossD_green = self.dis_loss_green.eval(feed_dict=feed_dic)
        lossD_blue = self.dis_loss_blue.eval(feed_dict=feed_dic)
        lossD = self.dis_loss.eval(feed_dict=feed_dic) + lossD_saturation + lossD_hue + lossD_red + lossD_green + lossD_blue
