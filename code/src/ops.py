...
def pixelwise_accuracy(img_real, img_fake, colorspace, thresh, options=None):
    """
    ...
    if options is None:
        ...
        img_real_rgb = postprocess(img_real, colorspace, COLORSPACE_RGB)
        img_fake_rgb = postprocess(img_fake, colorspace, COLORSPACE_RGB)

        img_real_hsv = tf.image.rgb_to_hsv(img_real_rgb)
        img_fake_hsv = tf.image.rgb_to_hsv(img_fake_rgb)

        # Apply HSV changes to saturation and others in here -- but pay attention; img_real and img_fake are in LAB
        img_real_hue = tf.reduce_mean(img_real_hsv[..., 0])
        img_real_saturation = tf.reduce_mean(img_real_hsv[..., 1])

        img_real_rgb = tf.image.adjust_hue(img_real_rgb, options.desired_hue / img_real_hue)
        img_real_rgb = tf.image.adjust_saturation(img_real_rgb, options.desired_saturation / img_real_saturation)
        # img_real_hsv[..., 0] = options.hue_weight*(img_real_hsv[..., 0] + (options.desired_hue - img_real_hue))
        # img_real_hsv[..., 1] = options.saturation_weight*(img_real_hsv[..., 1] + (options.desired_saturation - img_real_saturation))

        img_real_rgb = tf.image.hsv_to_rgb(img_real_hsv)
        img_fake_rgb = tf.image.hsv_to_rgb(img_fake_hsv)

        img_real = rgb_to_lab(img_real_rgb)
        img_fake = rgb_to_lab(img_fake_rgb)
